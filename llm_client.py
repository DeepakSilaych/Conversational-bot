...
class LLMClient:
    def __init__(self, 
        ...
        self.insurance_summary = None
        self.insurance_summary_hash = None
    ...
    async def _lazy_load_lead_data(self):
        ...
        try:
            ...
            if res.data:
                ...
                # Store recommended_plans for later use
                if existing.get("recommended_plans"):
                    self.recommended_plans = existing["recommended_plans"]
                    print(f"[RESTORE] Found recommended_plans: {self.recommended_plans}")
            ...
            # Process conversation state - THIS IS THE AUTHORITATIVE SOURCE
            phase_loaded_from_db = False
            if conversation_state.data and conversation_state.data[0]["collected_info"] is not None:
                self.collected_info = conversation_state.data[0]["collected_info"]
                self.is_all_info_collected = conversation_state.data[0].get("is_all_info_collected", False)
                print(f"Self Collected info from Supabase : {self.collected_info} : {self.is_all_info_collected}")
                
                # Restore conversation phase from database - THIS IS THE SOURCE OF TRUTH
                if conversation_state.data[0].get("current_phase"):
                    self.conversation_phase = conversation_state.data[0]["current_phase"]
                    phase_loaded_from_db = True
                    print(f"[RESTORE] ✅ Loaded conversation phase from DB: {self.conversation_phase}")
                    
                    # If we're in post_reco, we need to load plan context
                    if self.conversation_phase == "post_reco":
                        print(f"[RESTORE] Phase is post_reco, marking for plan context load")
                        self.needs_plan_context = True
                        
                        # Also restore other post_reco related data if available
                        if conversation_state.data[0].get("recommendation_data"):
                            self.recommendation_data = conversation_state.data[0]["recommendation_data"]
                            print(f"[RESTORE] Loaded recommendation_data from DB")
            ...
            # ONLY determine phase if we didn't load it from DB
            if not phase_loaded_from_db:
                print(f"[PHASE] No phase in DB, determining from data...")
                
                # Check if lead has recommendations
                ...
                # Save the determined phase
                asyncio.create_task(self._save_conversation_state_async())
            else:
                print(f"[PHASE] Using phase from DB: {self.conversation_phase}")
        ...
        # Load plan context if needed (do this AFTER is_loaded = True)
        if getattr(self, 'needs_plan_context', False):
            print(f"[RESTORE] Loading plan context for post_reco phase...")
            await self._load_plan_context_for_sales()
            self.needs_plan_context = False

    async def _get_insurance_summary(self, force_refresh=False):
        """Get cached insurance summary or generate if needed"""
        
        # Create hash of collected_info to detect changes
        current_hash = hash(json.dumps(self.collected_info, sort_keys=True))
        
        # Return cached if no changes and not forced
        if not force_refresh and self.insurance_summary and self.insurance_summary_hash == current_hash:
            return self.insurance_summary
        
        # Build members array with proper format
        members_covered = []
        if self.collected_info.get("who_to_insure") and self.collected_info.get("ages"):
            for member in self.collected_info["who_to_insure"]:
                member_data = {
                    "relationship": member,
                    "age": self.collected_info["ages"].get(member, 0)
                }
                # Add gender if available
                if self.collected_info.get("genders", {}).get(member):
                    member_data["gender"] = self.collected_info["genders"][member]
                members_covered.append(member_data)
    
        # Get values
        requested_budget = int(self.collected_info.get("budget", 0))
        requested_sum_assured = int(self.collected_info.get("sum_assured", 0))
        ...
        # Build complete summary matching frontend format
        self.insurance_summary = {
            "membersCovered": members_covered,  # Note: membersCovered, not members
        ...
        self.insurance_summary_hash = current_hash
        return self.insurance_summary
    
    async def _handle_button_ui_action(self, user_text: str) -> bool:
        """Handle pipe-separated UI button actions"""
        if "||" not in user_text:
            return False

        # ENSURE LEAD DATA IS LOADED BEFORE PROCESSING BUTTON ACTIONS
        await self._lazy_load_lead_data()
            
        parts = user_text.split("||")
        
        # Handle "compare||||selected" or "addToCart||||selected" format
        if len(parts) == 3 and parts[1] == "" and parts[2] == "selected":
            print(f"[UI BUTTON] Detected {parts[0]} selected action")
            
            ui_action = {
                'detected': True,
                'type': f'{parts[0].lower()}_selected',
                'plans': []
            }
            
            insurance_summary = await self._get_insurance_summary()
            await self._handle_ui_action_for_frontend(ui_action, insurance_summary)
            return True
        ...
        print(f"[UI BUTTON] Detected button action: {action_type} - {product_id} - {sub_action}")
        
        ui_action = {
            'detected': True,
            'type': None,
            'plans': [{'id': product_id, 'name': '', 'company': ''}],
            'button_action': True  # Mark as button action
        }
        
        # Map button actions
        if action_type == "compare":
            if sub_action == "add":
                ui_action['type'] = 'compare_add'
            elif sub_action == "remove":
                ui_action['type'] = 'compare_remove'
        elif action_type == "addtocart":
            if sub_action == "add":
                ui_action['type'] = 'cart_add'
            elif sub_action == "remove":
                ui_action['type'] = 'cart_remove'
    ...
    def _fast_parse_response(self, full_message: str) -> dict:
        ...
        # Check for dual format FIRST
        has_dual_format = False
        working_message = full_message
        
        if "DISPLAY:" in full_message and "TTS:" in full_message:
            try:
                # Extract DISPLAY content
                display_start = full_message.find("DISPLAY:") + 8
                tts_start = full_message.find("TTS:")
                
                if tts_start > display_start:
                    display_content = full_message[display_start:tts_start].strip()
                    
                    # Extract TTS content - but stop at ExtractedInfo if present
                    tts_content_start = tts_start + 4
                    extracted_info_pos = full_message.find("ExtractedInfo", tts_content_start)
                    if extracted_info_pos != -1:
                        tts_content = full_message[tts_content_start:extracted_info_pos].strip()
                        # For further parsing, use content from ExtractedInfo onwards
                        working_message = full_message[extracted_info_pos:]
                    else:
                        tts_content = full_message[tts_content_start:].strip()
                        working_message = ""  # No ExtractedInfo section
                    
                    result['message'] = display_content
                    result['tts_message'] = tts_content
                    has_dual_format = True
                    
                    print(f"[PARSE DEBUG] Extracted DISPLAY: {display_content[:50]}...")
                    print(f"[PARSE DEBUG] Extracted TTS: {tts_content[:50]}...")
            except Exception as e:
                print(f"[PARSE DEBUG] Dual format extraction failed: {e}")
                has_dual_format = False
        
        # If no dual format, DON'T set tts_message to full_message yet!
        if not has_dual_format:
            # We'll set these AFTER we extract the clean message
            result['message'] = full_message  # Temporary
            result['tts_message'] = ''  # Empty for now
        
        # Handle ExtractedInfo parsing
        if "ExtractedInfo" in working_message:
            print(f"[PARSE DEBUG] Found 'ExtractedInfo' in message")
            
            if not has_dual_format:
                # Only parse message part if we didn't already extract it from dual format
                ...
                if "Message:" in message_part:
                    ...
                    if len(parts_by_message) == 2:
                        ...
                        # IMPORTANT: Set TTS message to the CLEAN extracted message
                        result['tts_message'] = result['message']
                else:
                    ...
                    # Set TTS message to the CLEAN extracted message
                    result['tts_message'] = result['message']
            
            # Extract JSON from ExtractedInfo section
            parts = working_message.split("ExtractedInfo", 1) if has_dual_format else full_message.split("ExtractedInfo", 1)
            if len(parts) > 1:
            ...
            if not has_dual_format:
                # Clean the message by removing Message: prefix if it exists
                clean_msg = full_message.strip()
                if clean_msg.startswith("Message:"):
                    clean_msg = clean_msg[8:].strip()
                
                result['message'] = clean_msg
                result['tts_message'] = clean_msg  # Same for TTS
        ...
        # If tts_message is STILL empty, use message as fallback
        if not result['tts_message'] and result['message']:
            result['tts_message'] = result['message']

        # At the very end, before returning - clean both messages
        result['message'] = clean_text(result['message'])
        result['tts_message'] = clean_text(result['tts_message'])
        
        print(f"[PARSE FINAL] ===== PARSING COMPLETE =====")
        print(f"[PARSE FINAL] Final message: {result['message'][:100]}...")
        print(f"[PARSE FINAL] Final TTS message: {result['tts_message'][:100]}...")
        print(f"[PARSE FINAL] Final JSON: {result['json']}")
        print(f"[PARSE FINAL] Flags: {result['flags']}")
        ...
        try:
            ...
            if recommendation_response["type"] == "recommendations":
               ...
               #    })
                ...
                # Get properly formatted insurance summary
                insurance_summary = await self._get_insurance_summary()
               ...
               #}
                ...
                asyncio.create_task(self._save_transcript_async([result_json]))
    ...
    async def _save_conversation_state_async(self, extra_fields=None):
        ...
        try:
            ...
            # CRITICAL: Don't save if we haven't loaded data yet
            if not self.is_loaded:
                print(f"[DB SAVE] ⚠️ SKIPPING - data not loaded yet (is_loaded=False)")
                return
            
            # Validate lead_id
            if not self.lead_id or self.lead_id in ["null", "undefined", ""]:
                print(f"[DB SAVE] Skipping save - invalid lead_id: '{self.lead_id}'")
                return
            
            # CRITICAL: Don't save empty collected_info if we're in post_reco
            if self.conversation_phase == "post_reco" and not self.collected_info:
                print(f"[DB SAVE] ❌ ERROR: Attempting to save empty collected_info in post_reco phase!")
                print(f"[DB SAVE] This would corrupt the database - ABORTING SAVE")
                return
                
            # FIX: Ensure consistency between phase and is_all_info_collected
            if self.conversation_phase == "post_reco":
                self.is_all_info_collected = True  # Must be true if we're in post_reco
                print(f"[DB SAVE] Fixed is_all_info_collected to True for post_reco phase")
            elif self.conversation_phase == "ready_for_reco":
                self.is_all_info_collected = True  # Also true for ready_for_reco
                print(f"[DB SAVE] Fixed is_all_info_collected to True for ready_for_reco phase")
            
            # Check if we should use UPDATE instead of UPSERT
            ...
            existing = await loop.run_in_executor(
                ...
                lambda: self.supabase.from_("conversation_states")
                    ...
                    .maybe_single()  # ← Returns None if no rows, data if 1 row
                    .execute()
            )
            ...
            data = {
                ...
                "unanswered_questions": getattr(self, 'unanswered_questions', []),
            ...
            # If record exists and has collected_info, but we're about to save empty, skip
            if existing and existing.data and existing.data.get("collected_info") and not self.collected_info:
                print(f"[DB SAVE] ❌ WARNING: Existing record has collected_info but we have empty - SKIPPING SAVE")
                return
            
            if existing and existing.data:
                # Record exists - UPDATE only (safer!)
                result = await loop.run_in_executor(
                    db_executor,
                    lambda: self.supabase.from_("conversation_states")
                        .update(data)  # Only updates specified fields
                        .eq("lead_id", self.lead_id)
                        .execute()
                )
                print(f"[DB SAVED] Updated existing record")
            else:
                # Record doesn't exist - INSERT with lead_id
                ...
                print(f"[DB SAVED] Created new record")
    ...
    async def _save_transcript_async(self, transcript_json):
        """Save transcript in lead_notes to DB without blocking the event loop."""
        note = {
            "lead_id": self.lead_id,
            "content": "",
            "type": "web_call",
            ...
            "transcript": json.dumps(transcript_json, ensure_ascii=False),
        }

        def _insert():
            # This runs in a thread; OK to do blocking I/O here
            return self.supabase.from_("lead_notes").insert([note]).execute()
 ...
 #       best_diff = float('inf')
  ...
  #     self.best_plan_id = best_plan_id or list(self.plan_details.keys())[0]
    ...
    def _extract_plan_names_from_text(self, text: str) -> list:
        ...
        for plan_id, details in sorted_plans:
            ...
            for variant in variations:
                ...
                # For variants with "+", use lookahead/lookbehind instead of \b
                if '+' in variant:
                    # Match "Diamond+" with space or end of string after +
                    pattern = rf'(?<!\w){escaped}(?=\s|$|[,.\-])'
                else:
                    # Regular word boundary for variants without +
    ...
    async def _handle_ui_action_for_frontend(self, ui_action: dict, insurance_summary: dict = None):
        """Format data using format_response, then send via _on_plans"""
        
        # ========== COMPARE ACTIONS ==========
        if ui_action['type'] in ['compare_add', 'compare_remove']:
            # Button-based compare add/remove - send short update
            plan_ids = [p['id'] for p in ui_action['plans']]
            print(f"[UI DEBUG] Compare {ui_action['type']} plan_ids: {plan_ids}")
            
            # Update DB
            if ui_action['type'] == 'compare_remove':
                await self._remove_from_compare_in_db(plan_ids)
            else:  # compare_add
                await self._update_compare_in_db(plan_ids)
            
            # Get updated compare list
            loop = asyncio.get_event_loop()
            result = await loop.run_in_executor(
                db_executor,
            ...
            compare_list = []
            if result.data and result.data.get('compare'):
                compare_list = result.data['compare'].split(',') if result.data['compare'] else []
            
            # Send short update
            result = {
                "type": "compare_update",
                "data": {
                    "itemsSelected": len(compare_list),
                    "itemIds": compare_list
                },
                "message": f"{len(compare_list)} plan{'s' if len(compare_list) != 1 else ''} selected for comparison",
                "inline": False  # ADD THIS - UI button action
            }
            
            formatted = format_response(result, insurance_summary, True)

            asyncio.create_task(self._save_transcript_async([formatted]))

            # REMOVED: formatted['itemIds'] = compare_list
            # REMOVED: formatted['itemsSelected'] = len(compare_list)
            await self._on_plans(formatted)
            
        elif ui_action['type'] == 'compare_selected':
            # Button "selected" - show full comparison
            loop = asyncio.get_event_loop()
            result = await loop.run_in_executor(
                db_executor,
                lambda: self.supabase.from_('leads_selections')
            ...
            if result.data and result.data.get('compare'):
                plan_ids = result.data['compare'].split(',')
                ...
                print(f"[UI ACTIONS] Compare Selected Data {result}")
                formatted = format_response(result, insurance_summary, True)

                # REMOVED: formatted['itemIds'] = plan_ids
                # REMOVED: formatted['itemsSelected'] = len(plan_ids)
                await self._on_plans(formatted)
            else:
                result = {
                    "type": "text",
                    "message": "Please select plans to compare first",
                    "data": None,
                    "inline": False  # ADD THIS - UI button action
                }
                formatted = format_response(result, insurance_summary, True)
        ...
        # ========== NATURAL LANGUAGE COMPARE (keep existing) ==========
        elif ui_action['type'] == 'compare' and ui_action.get('plans'):
            # This is your existing natural language compare logic
            plan_ids = [p['id'] for p in ui_action['plans']]
            print(f"[UI DEBUG] Natural language compare plan_ids: {plan_ids}")
        ...
        # ========== CART BUTTON ACTIONS (new) ==========
        elif ui_action['type'] in ['cart_add', 'cart_remove'] and ui_action.get('button_action', False):
            # Button-based cart add/remove - send short update
            ...
            result = {
                ...
                "message": f"{len(self.cart_items)} plan{'s' if len(self.cart_items) != 1 else ''} in cart",
                "inline": False  # ADD THIS - UI button action
            }
            ...
            # REMOVED: formatted['itemIds'] = self.cart_items
            # REMOVED: formatted['itemsSelected'] = len(self.cart_items)
            await self._on_plans(formatted)
            
        elif ui_action['type'] == 'addtocart_selected':
            # Button "selected" - show full cart
            loop = asyncio.get_event_loop()
            result = await loop.run_in_executor(
                db_executor,
                lambda: self.supabase.from_('leads_selections')
            ...
            if result.data and result.data.get('cart'):
                self.cart_items = result.data['cart'].split(',')
                
                # Build full cart data
                cart_data = await self._build_cart_data_for_frontend(self.cart_items)
                
                if cart_data:
                    result = {
                        "type": "cart",
                        "data": cart_data,
                        "message": f"You have {len(self.cart_items)} plan{'s' if len(self.cart_items) > 1 else ''} in your cart",
                        "itemIds": self.cart_items,
                        "itemsSelected": len(self.cart_items), # ADD THIS LINE
                        "inline": False  # ADD THIS - UI button action
                    }
                    formatted = format_response(result, insurance_summary, True)
                    # REMOVED: formatted['itemIds'] = self.cart_items
                    ...
                    result = {
                        "type": "text",
                        "message": "Unable to load cart details",
                        "data": None,
                        "inline": False  # ADD THIS - UI button action
                    }
                    formatted = format_response(result, insurance_summary, True)
                ...
                formatted = format_response(result, insurance_summary, True)
                # REMOVED: formatted['itemsSelected'] = 0
                await self._on_plans(formatted)
                
        # ========== NATURAL LANGUAGE CART (keep your existing code) ==========
        elif ui_action['type'] in ['cart_add', 'cart_remove', 'show_cart']:
            # This is your existing natural language cart logic - keep it as is
            ...
            if cart_data:
                result = {
                    ...
                    "data": cart_data,
                    "message": f"You have {len(self.cart_items)} plan{'s' if len(self.cart_items) > 1 else ''} in your cart",
                    "itemIds": self.cart_items, # ADD THIS for frontend cart count
                    "itemsSelected": len(self.cart_items)  # ADD THIS LINE
                }
            else:
                result = {
    ...
    async def _build_comparison_data_for_frontend(self, plan_ids):
        ...
        # If in post_reco phase but no plan_details, load them
        if self.conversation_phase == "post_reco" and (not hasattr(self, 'plan_details') or not self.plan_details):
            await self._load_plan_context_for_sales()
            print("AM HERE2")

        # Fetch logos first
        logo_map = await self._fetch_logos_for_plans(plan_ids)
        headers = [{
    ...
    async def _build_cart_data_for_frontend(self, cart_plan_ids):
        """Build cart data matching edge function's formatCartProducts structure"""
        ...
        if not cart_plan_ids:
            return []
        
        # Fetch logos
        logo_map = await self._fetch_logos_for_plans(cart_plan_ids)
        
        grouped_by_company = {}
        company_order = []
        
        for plan_id in cart_plan_ids:
            if plan_id not in self.plan_details:
                continue
                
            details = self.plan_details[plan_id]
            company = details['company']
            short_company = COMPANY_SHORT_NAMES.get(company, company)
            is_direct = "Direct" in details['name']
            
            # Create item data
            item_data = {
                "variant_name": "Direct" if is_direct else "Regular",
                "product_id": plan_id,
                "product_name": details['name'],
                "usp": details['usps'][:3] if details.get('usps') else [],
                "premium": details['current_premium'],
                "discount": 0,
                "sum_assured": details.get('sum_assured'),
                "base_product_name": details['name'],
                "tenure": 1,
                "buttons": [
                    {
                        "label": "Buy Now",
            ...
            if company not in grouped_by_company:
                # First product for this company - featured
                grouped_by_company[company] = {
                    "company": short_company,
                    "company_logo": logo_map.get(plan_id, ""),  # Same approach!
                    "base_product_name": details['name'],
                    "sum_assured": details.get('sum_assured'),
                    ...
                    "tenure": 1,
                    "featured": True,
                    "buttons": item_data["buttons"],
                    "sub_items": []
                }
                company_order.append(company)
            else:
                # Additional product - add to sub_items
                item_data["featured"] = False
                grouped_by_company[company]["sub_items"].append(item_data)
        
        # Convert to array preserving cart order
        cart_items_formatted = []
        for plan_id in cart_plan_ids:
            if plan_id in self.plan_details:
                company = self.plan_details[plan_id]['company']
                if company in grouped_by_company and grouped_by_company[company] not in cart_items_formatted:
                    cart_items_formatted.append(grouped_by_company[company])
        
        return cart_items_formatted

    async def _fetch_logos_for_plans(self, plan_ids):
    ...
    async def _build_cart_data_for_frontend(self, cart_plan_ids):
        ...
        for plan_id in cart_plan_ids:
            ...
            item_data = {
                ...
                "premium": details['current_premium'],
                "discount": 0,
                "sum_assured": details.get('sum_assured', self.collected_info.get('sum_assured', 1000000)),
                "base_product_name": details['name'],
                "tenure": 1,
                "buttons": [
                    {
            ...
            if company not in grouped_by_company:
                ...
                grouped_by_company[company] = {
                    "company": short_company,
                    "company_logo": logo_map.get(plan_id, ""),
                    "base_product_name": details['name'],
                    "sum_assured": details.get('sum_assured', self.collected_info.get('sum_assured', 1000000)),
                    "variant_name": item_data["variant_name"],
                    "product_id": plan_id,
    ...
    async def _remove_from_compare_in_db(self, plan_ids_to_remove):
        """Remove plans from compare list in DB"""
        try:
            loop = asyncio.get_event_loop()
            ...
            if result.data and result.data.get('compare'):
                current_list = result.data['compare'].split(',')
                # Remove the plans
                updated_list = [pid for pid in current_list if pid not in plan_ids_to_remove]
                
                # Update DB
                await loop.run_in_executor(
                    db_executor,
                    lambda: self.supabase.from_('leads_selections')
                        .update({'compare': ','.join(updated_list)})
                        .eq('lead_id', self.lead_id)
                        .eq('chat_id', '1')
                        .execute()
                )
                print(f"[DB] Updated compare: {len(updated_list)} items after removal")
            
        except Exception as e:
            print(f"[DB ERROR] Failed to remove from compare: {e}")
    ...
    async def _update_compare_in_db(self, plan_ids):
        ...
        try:
            ...
            # First, get the current compare list
            result = await loop.run_in_executor(
                db_executor,
                lambda: self.supabase.from_('leads_selections')
                    .select('compare')
            ...
            existing_compare = []
            if result.data and result.data[0].get('compare'):
                existing_compare = result.data[0]['compare'].split(',')
            
            # Add new plan_ids to existing list (avoid duplicates)
            for plan_id in plan_ids:
                if plan_id not in existing_compare:
                    existing_compare.append(plan_id)
            
            # Prepare data
            compare_data = {
                'lead_id': self.lead_id,
                'chat_id': '1',
                'compare': ','.join(existing_compare)
            }
            ...
            if result.data:
                ...
                await loop.run_in_executor(
                    db_executor,
                    lambda: self.supabase.from_('leads_selections')
                        .update({'compare': ','.join(existing_compare)})
                        .eq('lead_id', self.lead_id)
                        .eq('chat_id', '1')
                        .execute()
                )
            ...
            self.compare_items = existing_compare
    ...
    async def stream_response(self, user_text: str) -> AsyncGenerator[str, None]:
            ...
            # ============ HANDLE BUTTON UI ACTIONS ============
        # Check for button clicks BEFORE storing in history
        if await self._handle_button_ui_action(user_text):
            # Button action was handled, don't process further
            yield ""  # Empty yield to complete the generator
            return
        # ============ END BUTTON UI ACTIONS ============

        # Store user message in history 
        # COMMENTING AS THIS IS ADDING TEXT TWICE IN HISTORY AND TRANSCRIPT
        # print(f"[TRANSCRIPT] Adding User text: {user_text}")
        ...
        try:
            ...
            if self.conversation_phase == "post_reco":
                ...
                if self.post_reco_interaction_count == 0:
                    ...
                    # Send nudge through on_plans for display (MESSAGE PIPE)
                    insurance_summary = await self._get_insurance_summary()
            ...
            # Parse response
            parsed_response = self._fast_parse_response(full_message)
            # Store TTS version for server.py to access
            self.current_tts_message = parsed_response.get('tts_message', '')
            extracted_message = parsed_response['message']  # This is now the DISPLAY version
            extracted_json = parsed_response['json']
            flags = parsed_response['flags']
            ...
            print(f"[Chat msg]: {clean_message}")
            print(f"[TTS msg]: {self.current_tts_message}")
            for token in  self.current_tts_message.split():
                yield token + " "
            
            # In stream_response, fix the undefined insurance_summary:
            if clean_message and clean_message.strip():
                insurance_summary = await self._get_insurance_summary()  # Add this line
                await self._on_plans({
                    ...
                    "from": "agent",
                    "showMessage": True,
                    "insuranceSummary": insurance_summary,  # Now it's defined
                    "time": int(time.time() * 1000)
                })
            ...
            if self.conversation_phase == "post_reco" and hasattr(self, 'plan_details') and self.plan_details:
                try:
                    ...
                    if ui_action['detected']:
                        ...
                        # Get insurance summary ONCE here
                        insurance_summary = await self._get_insurance_summary()
